...
public class TestPlayActivity extends BaseAppCompatActivity implements View.OnClickListener, AnimationAndMusic.AnimationStopInterface {
    public MediaPlayer player;
    private String TAG = "TestPlayActivity";
    private String gateNum;
    private String userIdJiequ;
    ...
    public void initData() {
        ...
        initView();
        new AnimationAndMusic(TestPlayActivity.this, animationView, "qian_mo3.mp3", R.drawable.push_animation_3, 1);
        StatisticsUtil.getStatistics(TestPlayActivity.this, Const.STR14);
    }
    ...
    private void initView() {
        ...
        speakericon = PreferencesUtil.getBoolean(TestPlayActivity.this, "speakericon");
        boolean specialEffects = PreferencesUtil.getBoolean(TestPlayActivity.this, "special_effects");
        String url;
        if (specialEffects) {
            url = ContactUrl.POST_PLAY_TEST_PATH + "uid=" + Const.UID + "&token=" + Const.TOKEN + "&gateNum=" + gateNum + "&skinId=" + Const.SKINID + "&effectId=" + Const.STR0;
        } else {
            url = ContactUrl.POST_PLAY_TEST_PATH + "uid=" + Const.UID + "&token=" + Const.TOKEN + "&gateNum=" + gateNum + "&skinId=" + Const.SKINID + "&effectId=" + Const.STR0;
        }
        wvTask.loadUrl(url);
        ...
        wvTask.setWebViewClient(new WebViewClient() {
            ...
            public boolean shouldOverrideUrlLoading(WebView view, String url) {
                if (url.contains("myschema://go?a=11")) {
                    ...
                    Intent intent = new Intent(TestPlayActivity.this, ChessLoginTypeActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                    startActivity(intent);
                    return true;
                } else if (url.contains("myschema://go?a=5")) {
                    userIdJiequ = url.substring(20);
                    if (!TextUtils.isEmpty(gateNum) && !TextUtils.isEmpty(userIdJiequ)) {
                        getinsertSekiguchiRecord(userIdJiequ);
                    }
                    return true;
    ...
    //添加关卡记录
    private void getinsertSekiguchiRecord(String testScore) {
        try {
            OkHttpUtils.post().url(ContactUrl.POST_INSERT_RECORD_PATH)
                    .addParams("gateNum", gateNum)
                    .addParams("seat", Const.STR5)
                    .addParams("testScore", testScore)
                ...
                public void onResponse(String response, int id) {
                    BeasBean beasBean = GsonUtil.gsonToBean(response, BeasBean.class, TestPlayActivity.this);
                    if (beasBean == null) {
                        ToastUtils.show(Const.CONS_STR_ERROR);
                    } else {
                        boolean twoClassGuide = PreferencesUtil.getBoolean(TestPlayActivity.this, "twoClassGuide");
                        boolean oneClassGuideIsOver = PreferencesUtil.getBoolean(TestPlayActivity.this, "oneClassGuideIsOver");
                        if (!twoClassGuide && oneClassGuideIsOver) {
                            Intent intent = new Intent(TestPlayActivity.this, MainActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                            startActivity(intent);
                        }
                    }
                    finish();
