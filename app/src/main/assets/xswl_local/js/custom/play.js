...
var vmGameInfo = new Vue({
	...
	methods : {
		...
		enterGame(gameId, otherId) {
	        ...
	        if(vmGameInfo.currentGame.GameType == 4 && myRole == "white"){//AI执黑
	        eve.f("HandlerAction","B[pp]")();
	        }
	        if(vmGameInfo.currentGame.GameType == 4 && myRole == "black" && vmGameInfo.currentGame.RuleType1 >=2 ){
	        //让子棋AI执白先行
	        setTimeout(function(){
	...
	watch:{
		game:function(val){
			...
			//显示 9 路棋盘胜利规则
			if(blackLevel>=16 && whiteLevel>=16&&RuleType2==2){
				if(blackLevel>=21){
					$('.tzinfo').text('提3子胜')
				}else{
					$('.tzinfo').text('提7子胜')
...
eve.on("_ShowTake_", function(takeBlack, takeWhite) {
	...
	/*if(initMusic>0 && Tool.queryString('effectId')==0){//落子
